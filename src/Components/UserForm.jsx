...
export default function UsersPage() {
  ...
  const handleSave = async (payload) => {
    try {
      if (editing) {
        await apiFetch(`/api/users/${editing._id}`, { method: "PUT", body: JSON.stringify(payload) });
      } else {
  ...
  return (
    <div>
      ...
      {showForm && <UserForm user={editing} onCancel={() => setShowForm(false)} onSaved={handleSave} />}
    </div>
  );
}